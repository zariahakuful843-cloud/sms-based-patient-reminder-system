...
const NAV_ITEMS: NavItem[] = [
  ...
  },
  {
    label: "Patients",
    href: "/patients",
    roles: ["MEDICAL_RECORDS_OFFICER", "NURSE", "DOCTOR"],
    icon: (
  ...
  },
  {
    label: "SMS & Reminders",
    href: "/sms",
    roles: ["ADMIN", "MEDICAL_RECORDS_OFFICER"],
    icon: (
      <svg className="h-4 w-4" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
  ...
  return (
    <aside className="flex h-full w-60 flex-col bg-slate-900 text-slate-100">
      <div className="flex items-center gap-3 border-b border-slate-700 px-5 py-4">
        ...
        <div>
          <p className="text-xs font-bold text-white leading-tight">SMS Reminder</p>
          <p className="text-xs text-slate-400">Health System</p>
        </div>
      </div>
      ...
      <nav className="flex-1 overflow-y-auto py-3 px-3">
        {visibleItems.map((item) => {
          ...
          const label =
            role === "DOCTOR" && (item.label === "Patients" || item.label === "Appointments")
              ? `My ${item.label}`
              : item.label;
          return (
            <Link
              key={`${item.href}-${item.roles?.join("|") ?? "all"}`}
              href={item.href}
              className={cn(
