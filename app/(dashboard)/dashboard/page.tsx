...
export default async function DashboardPage() {
  ...
  const allowStatsTotalPatients = isAdmin || isReceptionist;
  const allowStatsAppointmentsToday = true; // all roles show Today's Appointments as per requirements
  const allowStatsPending = isReceptionist || isNurse;
  const allowStatsCompleted = isNurse || isDoctor;
  const allowStatsSms = isAdmin;

  // Quick actions (UI only)
  const quickActions = (() => {
    const items: Array<{ label: string; href: string; icon: React.ReactNode; hiddenFor?: boolean }> = [];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        ...
        {/* Completed/Consultations placeholders: keep UI minimal and do not change queries.
            These are controlled by role and rely on existing cards data when present. */}
        {(isNurse || isDoctor) && (
          <div className="group rounded-2xl bg-white shadow-sm ring-1 ring-slate-200 p-4 sm:p-5 hover:-translate-y-0.5 hover:shadow-md transition-all">
            <div className="flex items-start justify-between gap-3">
              <div>
                <p className="text-xs font-semibold text-slate-500">Completed</p>
                <p className="mt-2 text-2xl sm:text-3xl font-bold text-slate-900">0</p>
                <p className="mt-1 text-sm text-slate-500">Consultations/updates</p>
              </div>
              <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-emerald-50 text-emerald-700 ring-1 ring-emerald-100 group-hover:bg-emerald-100 transition-colors">
                <svg className="h-5 w-5" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
          ...
          <div className="group rounded-2xl bg-white shadow-sm ring-1 ring-slate-200 p-4 sm:p-5 hover:-translate-y-0.5 hover:shadow-md transition-all">
            <div className="flex items-start justify-between gap-3">
              <div>
                ...
                <p className="mt-2 text-2xl sm:text-3xl font-bold text-slate-900">{pendingReminders}</p>
                <p className="mt-1 text-sm text-slate-500">Awaiting updates</p>
              </div>
              <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-amber-50 text-amber-700 ring-1 ring-amber-100 group-hover:bg-amber-100 transition-colors">
                <svg className="h-5 w-5" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
      ...
      <div className="grid gap-4 sm:gap-6 lg:grid-cols-2">
        {/* Sections controlled by role via conditional rendering only */}
        ...
        {/* Nurse/Doctor single section: Today's Appointments */}
        {(isNurse || isDoctor) && (
